...
BASE_POPULATION_SCALE_FACTOR = 0.03  # For quick test simulations, this is set to a very low value
N_ITERATIONS = 2
...
config_templates = []
config_filename = os.path.join(template_files_dir, 'config_PrEPsweep.json')
...
def constrain_sample(sample):
    if ('ProbExtraTransitoryMaleLow' in sample) and ('ProbExtraTransitoryMaleMedium' in sample):
        sample['ProbExtraTransitoryMaleLow'] = min(sample['ProbExtraTransitoryMaleLow'], sample['ProbExtraTransitoryMaleMedium'])
    if ('ProbExtraTransitoryFemaleLow' in sample) and ('ProbExtraTransitoryFemaleMedium' in sample):
        sample['ProbExtraTransitoryFemaleLow'] = min(sample['ProbExtraTransitoryFemaleLow'], sample['ProbExtraTransitoryFemaleMedium'])
    return sample
...
def map_sample_to_model_input(sample_dict, template_set_name, scenario_name, campaign_filename, random_run_number=True):
    ...
    if ('PreARTLinkMinimum' in sample) and ('PreARTLinkMaximum' in sample):
        min_value = sample.pop('PreARTLinkMinimum')
        max_value = sample.pop('PreARTLinkMaximum')
