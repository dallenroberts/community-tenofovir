...
def analyze_experiments(experiment_managers, output_path):
    ...
    print('Experiments complete, analyzing...')

    am = AnalyzeManager(verbose=False)
    for em in experiment_managers:
        am.add_experiment(em.experiment)
    am.add_analyzer(DownloadAnalyzerTPI(filenames=['output\\ReportHIVByAgeAndGender.csv'],
...
def main(args):
    ...
    analyze_experiments(experiment_managers, output_path=args.output_path)
    print('Done!')
...
if __name__ == '__main__':
    ...
    # if script_args.scenario_mode == SCENARIO_TEMPLATE_SETS_MODE:
    #     if n_template_sets == 1:
    #         raise Exception('No non-Baseline template sets specified for scenario generation in provided calibration script.')
